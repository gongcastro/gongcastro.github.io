...
We may also assume that $x_0$, the mid-point of the logistic curve, equals zero. This means that we assume that the steepness of the curve will be maximum, and therefore its inflection point, when $x = 0$ (see @eq-mid and note how $x_0$ is no longer there after replacing it with 0).
...
\text{logistic}(x) &= \frac{1}{1 + e^{-\beta x}} \\
&= \frac{1}{1+e^{-x}} \\
&= \bigg(\frac{1}{1+e^{-\beta x}} \bigg) · \bigg( \frac{e^{\beta x}}{e^{\beta x}} \bigg) \\
& =\frac{e^{\beta x}}{e^{\beta x}+1} \\
&= \frac{e^{-\beta x}}{e^{-\beta x} + 1} \\
\end{aligned}
$$ {#eq-logistic-alternative}

The inverse of $e^{\beta x}$ is the natural logarithm of $\beta x$, that is, $\text{ln}(\beta x)$. The inverse of the logistic function as expressed in @eq-logistic-alternative, and therefore the logit function, is:
...
$$
p = \text{logistic}(x) = \frac{1}{1 + e^{-\beta x}}
$${#eq-logistic-logit}


There are more transformations available to make variables in the $[0, 1]$ interval suitable for linear regression, like *probit* or *cloglog*. We won't get into them, since they fall out of the scope of this blog post. Let's see how this takes the form of logistic regression in real data.
...
$$
\frac{p}{1-p} = \beta_{0} + \beta_{1} \times \text{Flipper length}
...
$$
p = \text{logistic}(x) = \frac{1}{1 + e^{-(\beta_0 + \beta_1 \times \ \text{Flipper length)}}}
