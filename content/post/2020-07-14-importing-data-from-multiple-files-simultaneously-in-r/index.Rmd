...
```{r timesplot, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Mean time (and standard deviation) for each combination of methods and file formats across 100 replications"}
# load packages
library(tidyverse)
library(data.table)
# import data
...
ggplot(dat, aes(package, mean, colour = vectorisation, shape = format)) +
  facet_wrap(~merge) +
  geom_line(aes(group = interaction(vectorisation, format)),
            position = position_dodge(width = 0.25)) +
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd),
                position = position_dodge(width = 0.25), linetype = "solid",
                width = 0) +
...
dat %>%
  ...
  tab_options(row_group.background.color = "#3b3d42",
