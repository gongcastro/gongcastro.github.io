...
description: "Using the Levenshtein distance to quantify the orthographic and phonlogical similarity between translation equivalents of the word *mask* across multiple languages."
date: '2020-11-20'
categories:
  - r
  - linguistics
...
dat <- read_xlsx(path = "mask.xlsx") %>%
  mutate(
    roman = as.logical(roman),
  ...
  drop_na(form) %>%
  filter(
    # other regions only provide missing values
    region %in% c("Europe", "Asia", "Africa", "Oceania"),
    # outlier, makes any other scores seem too close to each other (see masks.xlsx)
    language != "Hmong"
  ) %>%
  as_tibble()

data_link <- "https://drive.google.com/file/d/18SeJTiM2-JXR9SOqEg22wdkvNL3OxG3u/view?usp=sharing"
...
t <- dat %>%
  ...
  tab_source_note(source_note = html(data_link)) %>%
  cols_label(
...
expanded_ort <- expand_grid(label1 = dat$label, label2 = dat$label) %>%
  left_join(., {
    dat %>%
  ...
  mutate(
    n = ifelse(
      nchar(romanisation1) > nchar(romanisation2),
      nchar(romanisation1),
      nchar(romanisation2)
    ),
...
```{r}
#| label: phonology
expanded_phon <- expand_grid(
  label_phon1 = dat$label_phon,
  ...
  mutate(
    n = ifelse(
      nchar(ipa_flat1) > nchar(ipa_flat2),
      nchar(ipa_flat1),
      nchar(ipa_flat2)
    ),
...
onsets <- list(
  ...
  Phonology = dat %>%
    ...
    count(onset) %>%
    mutate(prop = n / sum(.$n), onset = paste0("/", onset, "/"))
) %>%
  bind_rows(.id = "type") %>%
  mutate(
    type = ifelse(
